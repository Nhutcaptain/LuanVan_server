import { adviseForHealthStatus } from "../config/services/gpt.service";
...
export const createHealthStatus = async (req: any, res: any) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Lỗi từ server khi tạo tình trạng sức khoẻ" });
  }
};
...
export const getHealthStatus = async (req: any, res: any) => {
  try {
    ...
    const status = await HealthStatus.findOne({ userId: id }).sort({
      createdAt: -1,
    });
    ...
    if (!status) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy tình trạng sức khoẻ" });
    }
    ...
    return res
      .status(500)
      .json({ message: "Lỗi khi lấy thông tin sức khoẻ từ server" });
  }
};
...
export const createExamination = async (req: any, res: any) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Lỗi server khi tạo bệnh án", error });
  }
};

export const updateHealthStatus = async (req: any, res: any) => {
  try {
    const { userId } = req.params;
    const updateField = req.body;

    if (!userId || !updateField || typeof updateField !== 'object') {
      return res.status(400).json({ message: "Thiếu thông tin" });
    }
    const keys = Object.keys(updateField);
    const isSimpleUpdateOnly = keys.every(key =>
      ['height', 'weight', 'blood'].includes(key)
    );

    let updatePayload = { ...updateField };

    if (!isSimpleUpdateOnly) {
      let existing = await HealthStatus.findOne({ userId });
      if (!existing) {
        existing = new HealthStatus({ userId });
      }
      const updatedData = {
        ...existing.toObject(),
        ...updateField,
      ...
      const aiNote = await adviseForHealthStatus(updatedData);
      updatePayload.note = aiNote;
    }

    const updatedStatus = await HealthStatus.findOneAndUpdate(
      { userId },
      { $set: updatePayload },
